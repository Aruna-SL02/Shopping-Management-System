...
namespace OnlineShop
{
    public partial class Login : Form
    {
        public Login()
        {
            InitializeComponent();
            /*
            // 1) Make the form open maximized and center on screen
            this.WindowState = FormWindowState.Normal;
            this.StartPosition = FormStartPosition.CenterScreen;

            // 2) Make the background image (pictureBox2) stretch to fill
            pictureBox2.Dock = DockStyle.Fill;
            pictureBox2.SizeMode = PictureBoxSizeMode.StretchImage;

            // 3) Dock the top panel (panel1) so it always stays at the top edge
            panel1.Dock = DockStyle.Top;
            
            // 4) Subscribe to the form's Resize event so panel3 is always centered
            this.Resize += new EventHandler(Form_Resize);

            // 5) Call Form_Resize once right away to center panel3 on startup
            Form_Resize(this, EventArgs.Empty); */
        }
        /*
        // Centers panel3 in the middle of the client area
